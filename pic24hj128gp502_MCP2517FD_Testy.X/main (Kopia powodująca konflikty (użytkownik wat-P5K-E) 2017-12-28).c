/*****************************************************************************
  ...
  Created on:      15 grudnia 2017, 13:02
 ******************************************************************************/
...
#define cREGADDR_CiFLTCON0   0x1D0
...
//! SPI Transmit buffer
char BuforTX[SPI_DEFAULT_BUFFER_LENGTH] __attribute__((space(dma)));
//! SPI Receive buffer
char BuforRX[SPI_DEFAULT_BUFFER_LENGTH] __attribute__((space(dma)));
/*Bufory pomocnicze*/
...
/*dane wpisujemy i odczytujemy z rejestru CiFLTCON0*/  
    /*rejestr sklada si? z 4 bajtów = 32bity*/
            uint8_t length ;
            
            /*tworzymy pojednyczy bajt rejestru CiFLTCON*/   
            REG_CiFLTCON_BYTE foo ;
            ...
            for (length = 0; length < 4; length++) {
                ...
                _Bool good = 0;
                for (length = 0; length < 4; length++) {
                    good = txd[length] == rxd[length];

                    if (!good) { /*jesli nieprawda to zapal diode*/
                       LED1_TOG ;
